...
public class Consumer implements Runnable{
    ...
    public Consumer(xwlTest xwltest) { 
	...
	public void run() {
		...
		while(true){
	        ...
	        //System.out.println(">>>>ICMP_pktQueue--"+ICMP_pktQueue.size());
	        Calendar calendar1 = Calendar.getInstance();	//获取当前时间
			long current = calendar1.getTimeInMillis();
			if((current-lastTime) >= 0 && (current-lastTime) < 7000){		// 时间片 = 1s, 30% * 1000 = 300
				...
				TCP_pktQueue.poll();
                TCP_infoQueue.poll();
			}else if((current-lastTime) >= 7000 && (current-lastTime) < 10000){
				...
				lastTime = calendar2.getTimeInMillis();
			}
